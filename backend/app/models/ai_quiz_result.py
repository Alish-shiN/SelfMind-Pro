...
class AIQuizResult(TimestampMixin, Base):
    ...
    insight: Mapped[str] = mapped_column(EncryptedText(), nullable=False)
    recommendation: Mapped[str] = mapped_column(EncryptedText(), nullable=False)
    practice: Mapped[str] = mapped_column(EncryptedText(), nullable=False)
    quiz_type: Mapped[str | None] = mapped_column(
        String(100), nullable=True, index=True
    )
    answers_snapshot: Mapped[list | None] = mapped_column(EncryptedJSON(), nullable=True)
    recommendations: Mapped[list | None] = mapped_column(EncryptedJSON(), nullable=True)
    micro_practices: Mapped[list | None] = mapped_column(EncryptedJSON(), nullable=True)
    action_plan: Mapped[dict | None] = mapped_column(EncryptedJSON(), nullable=True)
    ...
    trend_explanation: Mapped[str | None] = mapped_column(EncryptedText(), nullable=True)
